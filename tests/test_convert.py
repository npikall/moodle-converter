...
def test_pythonToMoodle():
    ...
    expected_output = "{rho}*(pow(ds,2)/(4*pow(rs,2)))*(((2*123*pow(c,2))/(pow(lam,5)))/(exp((123*123)/(lam*123*T))-1))*1*pow(10,24)"
    assert mc.python_to_moodle(mock_input, variable_map, constants_map) == expected_output